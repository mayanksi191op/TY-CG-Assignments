...
//		}
	...
	public boolean save(EmployeeTable e) {
		...
		EntityManagerFactory entityManagerFactory = Persistence.createEntityManagerFactory("myPersistenceUnit");
		EntityManager entityManager = entityManagerFactory.createEntityManager();
		String sqlString = "INSERT INTO EmployeeTable(employee_id, name, dob, joining_date, dept_id, designation, salary, manager_id, email, mobile, password) VALUES('"+e.getEmployee_id()+"', '"+e.getName()+"', '"+e.getDob()+"', '"+e.getJoining_date()+"', '"+e.getDept_id()+"', '"+e.getDesignation()+"', '"+e.getSalary()+", '"+e.getManager_id()+"', '"+e.getEmail()+"', '"+e.getMobile()+"', '"+e.getPassword()+"')";
	...
	public EmployeeTable get(int employee_id) {
		...
		System.out.println(employeeTable.getDept_id());
		employeeTable.setDesignation(list.get(0).getDesignation());
		System.out.println(employeeTable.getDesignation());
		employeeTable.setDob(list.get(0).getDob());
		System.out.println(employeeTable.getDob());
		employeeTable.setEmail(list.get(0).getEmail());
		System.out.println(employeeTable.getEmail());
		employeeTable.setEmployee_id(employee_id);
	...
	public boolean update(EmployeeTable e) {
		...
		try {
			...
			String sqlString = "update EmployeeTable set name = '"+e.getName()+"', dob = '"+e.getDob()+"', joining_date = '"+e.getJoining_date()+"', dept_id = '"+e.getDept_id()+"', designation = '"+e.getDesignation()+"', salary = '"+e.getSalary()+"', manager_id = '"+e.getManager_id()+"', email = '"+e.getEmail()+"', mobile = '"+e.getMobile()+"', password = '"+e.getPassword()+"') where employee_id = '"+e.getEmployee_id() + "'";
			Query query = entityManager.createQuery(sqlString, EmployeeTable.class);
			query.executeUpdate();
